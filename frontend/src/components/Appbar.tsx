...
const Appbar = memo(({searchBar, write, publish, edit, notifications}: {searchBar: boolean, write: boolean, publish: boolean, edit:boolean, notifications:boolean}) => {
    ...
    return (
        <div className={`${show ? "translate-y-0" : "-translate-y-full"} transition-transform duration-200 fixed z-50 w-full bg-white `}>
            <AppbarComponent searchBar={searchBar} write={write} edit={edit} publish={publish} notifications={notifications}/>
        </div>
    )
})
