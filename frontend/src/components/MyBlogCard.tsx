...
const MyBlogCard = memo(({myBlog, homeDraftsLibrary, atomNumber}: {myBlog: Blog, homeDraftsLibrary: "home" | "drafts" , atomNumber: number}) => {
    ...
    const likeCountFormatter = new Intl.NumberFormat('en', {
        ...
        if (myBlog.published==false && homeDraftsLibrary == "home"){
            return (
                <></>
            )
        }
        if (myBlog.published==true && homeDraftsLibrary == "drafts"){
            return (
                <></>
            )
        }
                ...
                <div className="max-w-3xl w-full mx- sm:mx-16 md:mx-16 lg:mx-0 border-b-1 border-slate-100 mt-4 pb-6">
                    ...
                    <div className="grid grid-cols-6 mt-2 hover:cursor-pointer" onClick={ () => {
                        ...
                        <div className="col-span-4 ">
                            <div className="text-md sm:text-lg md:text-2xl text-gray-800 font-bold">
                                {myBlog.title.length >= 94 ? `${myBlog.title.slice(0,94)}...` : myBlog.title}
                            </div>
                            <div className="mt-2 font-medium text-xs sm:text-sm md:text-md text-gray-600">
                                {myBlog.summary.length >= 120 ? `${myBlog.summary.slice(0,120)}...` : myBlog.summary}
                            </div>
                        </div>
                        <div className="col-span-2 flex justify-end">
                            <div>
                                {myBlog.imageExist ? <MyBlogsImage myBlogId={myBlog.id} atomNumber={atomNumber} /> : <div> </div>}
                            </div>
                        </div>
                    </div>
                    <div onClick={ () => {
                    ...
                    className=" flex mt-5 text-slate-600 text-xs font-semibold">
                        ...
                        <div className="flex justify-evenly gap-1 ">
                            ...
                            <div className="mt-1">{likeCountFormatter.format(myBlog.likes)}</div>
                        </div>
                        <div className="gap-1 flex">
                            ...
                            <div className="mt-1">{myBlog.numberOfComments}</div>
                        </div>
                    </div>
                </div>
            </div>
