...
const MyBlogCard = memo(({blog, homeDraftsLibrary}: {blog: Blog, homeDraftsLibrary: string}) => {
    ...
    const likeCountFormatter = new Intl.NumberFormat('en', {
        ...
        if (blog.published==false && homeDraftsLibrary == "home"){
            return (
                <></>
            )
        }
        if (blog.published==true && homeDraftsLibrary == "drafts"){
            return (
                <></>
            )
        }
            ...
            <div className=" flex justify-center mt-4">
                <div className="max-w-3xl w-full mx- sm:mx-16 md:mx-16 lg:mx-0 border-b-1 border-slate-100 mt-4 pb-6 hover:cursor-pointer">
                    <div className="flex text-sm">
                        <div className="bg-amber-400 text-center py-1 h-6 w-6 text-xs mr-2 rounded-full">
                            {blog.author.name[0]}
                        </div>
                    ...
                    <div className="grid grid-cols-6 mt-2 hover:cursor-pointer" onClick={ () => {
                        navigate(`/myBlog?blogId=${blog.id}`)
                        window.scrollTo(0,0);
                        setEditBlog(blog)
                    }}>
