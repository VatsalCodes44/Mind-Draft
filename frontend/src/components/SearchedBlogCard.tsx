...
import { searchedUserProfilePic, userProfileColor } from "../store/userInfo/atom";
import { searchedUserImageAtomFamily } from "../store/blogs/atom";
...
const SearchedBlogCard = memo(({userBlog, homeDraftsLibrary, atomNumber}: {userBlog: Blog, homeDraftsLibrary: "home" | "drafts" , atomNumber: number}) => {
    const navigate = useNavigate()
    const profilePic = useRecoilValue(searchedUserProfilePic)
    const color = useRecoilValue(userProfileColor)
    const images = useRecoilValue(searchedUserImageAtomFamily(atomNumber))
            ...
            <div className=" flex justify-center mt-4">
                <div className="max-w-3xl w-full mx- sm:mx-16 md:mx-16 lg:mx-0 border-b-1 border-slate-100 mt-4 pb-6 hover:cursor-pointer">
                    ...
                    <div className="grid grid-cols-6 mt-2 hover:cursor-pointer" onClick={ () => {
                        navigate(`/userBlog?blogId=${userBlog.id}&number=${atomNumber}`)
                        window.scrollTo(0,0);
                    }}>
