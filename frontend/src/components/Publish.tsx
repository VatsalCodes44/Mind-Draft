...
const Publish = memo(() => {
    const [title, setTitle] = useRecoilState(t)
    const [summary,setSummary] = useRecoilState(s)
    const [htmlContent, setHtmlContent] = useRecoilState(content)
    const editorState = useRecoilValue(eState)
    const setPreview = useSetRecoilState(preview)
    const [image ,setImage] = useRecoilState(getImage)
    const [imageExist, setImageExist] = useRecoilState(imgExist)
    ...
    const handleApi = useCallback((published: boolean) => {
        ...
        axios.post("http://localhost:8787/api/v1/blog/upload",formData, {
          headers: {
